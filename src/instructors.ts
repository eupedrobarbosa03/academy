...
class Instructor {
        ...
        return {
            name: () => {
                if (!this.inputName.value.match(academyRegex.name)) {
                    return Utils.showError("message-error-name-instructor", this.inputName.id,
    ...
    create() {
        ...
        buttonRegister.addEventListener("click", () => {
            if (!this.validations().name() || !this.validations().cpf() || !this.validations().telephone() || !this.validations().specialty()) return;
