        ...
        return {
            ...
            cpf: () => {
                if (!this.inputCPF.value.match(academyRegex.cpf)) {
                    return Utils.showError(
                        "message-error-cpf-instructor",
                        this.inputCPF.id,
                        `CPF inválido. Verifique o formato.`
                    );  
                };

                const instructors = storage.get<InstructorType[], "instructors">("instructors");
                if (!instructors) return null;
                const existingCFP = instructors.find((instructor) =>
                    instructor.cpf === this.inputCPF.value);

                if (existingCFP) return Utils.showError(
                    "message-error-cpf-instructor",
                    this.inputCPF.id,
                    "O CPF informado está em uso."
                )
